...
func Register(app *echo.Group) {
	...
	auth.GET("/verify/email/:email/:token", controller.Set[VerifyEmailTokenDto](VerifyEmail))
	auth.GET("/verify/password/:email/:token", controller.Set[VerifyEmailTokenDto](VerifyPassword))

	auth.POST("/login", controller.Set[LoginDto](Login))
	auth.POST("/signup", controller.Set[SignupDto](Signup))
	auth.POST("/forgot", controller.Set[ForgotPasswordDto](ForgotPassword))
